...
class _HddPageState extends State<HddPage> {
  ...
  searchListener() {
    if (showSearch) {
      hddState.search(searchController.text, true);
    }
  }
  ...
  toggleSearch() {
    setState(() {
      ...
      if (!showSearch) hddState.search(searchController.text, false);
    });
  }
  ...
  Widget listBuilder() {
    return RefreshIndicator(
      ...
      onRefresh: hddState.loadData,
      child: StreamBuilder<List<Part>>(
        stream: hddState.list,
  ...
  navigate2filterPage(BuildContext context) async {
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => HddFilterPage()),
    );
  }
}
